...
class ResShiftSampler(BaseSampler):
    def sample_func(self, y0, noise_repeat=False, mask=False):
        ...
        for out in self.base_diffusion.p_sample_loop_progressive(
            ...
            if step_count == mid_step:
                # decode_first_stage将latent还原到图像域
                with torch.no_grad():
                    im_mid = self.base_diffusion.decode_first_stage(out["sample"], self.autoencoder, None)
                    im_mid = im_mid.clamp(-1.0, 1.0)
        ...
        # 解码最终结果
        with torch.no_grad():
            im_sr = self.base_diffusion.decode_first_stage(final_sample, self.autoencoder, None)
            im_sr = im_sr.clamp(-1.0, 1.0)
        if flag_pad:
            im_sr = im_sr[:, :, :ori_h * self.sf, :ori_w * self.sf]
            if im_mid is not None:
                im_mid = im_mid[:, :, :ori_h * self.sf, :ori_w * self.sf]
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=True, bs=1, noise_repeat=False):
        ...
        def _process_per_image(im_lq_tensor, mask=None):
            ...
            if im_lq_tensor.shape[2] > self.chop_size or im_lq_tensor.shape[3] > self.chop_size:
                ...
                for im_lq_pch, index_infos in im_spliter:
                    ...
                    with context():
                        ...
                        #         mask=mask_pch,
                        #         )     # 1 x c x h x w, [-1, 1]
                        im_sr_pch, im_mid_pch = self.sample_func(
                            im_lq_pch,
                            noise_repeat=noise_repeat,
                            mask=mask_pch,
                        )
                ...
                # print(im_lq_tensor.shape)
                with context():
                    im_sr_tensor, im_mid_tensor = self.sample_func(
                        im_lq_tensor,
                        noise_repeat=noise_repeat,
                        mask=mask,
                    )
            ...
            im_sr_tensor = im_sr_tensor * 0.5 + 0.5
            if im_mid_tensor is not None:
                im_mid_tensor = im_mid_tensor * 0.5 + 0.5
            if mask_back and mask is not None:
                mask = mask * 0.5 + 0.5
                im_lq_tensor = im_lq_tensor * 0.5 + 0.5
                im_sr_tensor = im_sr_tensor * mask + im_lq_tensor * (1 - mask)
                if im_mid_tensor is not None:
                    im_mid_tensor = im_mid_tensor * mask + im_lq_tensor * (1 - mask)
            return im_sr_tensor,im_mid_tensor
        ...
        if self.rank == 0:
            ...
            if not out_path.exists():
                out_path.mkdir(parents=True)
            mid_out_path = out_path / 'mid_frames'
            if not mid_out_path.exists():
                mid_out_path.mkdir(parents=True)
        ...
        if in_path.is_dir():
            ...
            for data in dataloader:
                ...
                if micro_data['lq'].shape[0] > 0:
                    ...
                    #         mask=micro_data['mask'].cuda() if 'mask' in micro_data else None,
                    #         )    # b x h x w x c, [0, 1], RGB
                    im_sr_tensor, im_mid_tensor = _process_per_image(
                        micro_data['lq'].cuda(),
                        mask=micro_data['mask'].cuda() if 'mask' in micro_data else None,
                    )
                    ...
                    #     util_image.imwrite(im_sr, im_path, chn='bgr', dtype_in='uint8')
                    # 保存最终结果
                    for jj in range(im_sr_tensor.shape[0]):
                        im_sr = util_image.tensor2img(im_sr_tensor[jj], rgb2bgr=True, min_max=(0.0, 1.0))
                        im_name = Path(micro_data['path'][jj]).stem
                        im_path = out_path / f"{im_name}.png"
                        util_image.imwrite(im_sr, im_path, chn='bgr', dtype_in='uint8')
                        ...
                        # 如果有中间帧，则保存中间帧图像
                        if im_mid_tensor is not None:
                            im_mid = util_image.tensor2img(im_mid_tensor[jj], rgb2bgr=True, min_max=(0.0, 1.0))
                            im_mid_path = mid_out_path / f"{im_name}_mid.png"
                            util_image.imwrite(im_mid, im_mid_path, chn='bgr', dtype_in='uint8')
            if self.num_gpus > 1:
                dist.barrier()
        else:
            im_lq = util_image.imread(in_path, chn='rgb', dtype='float32')  # h x w x c
        ...
        self.write_log(f"Processing done, enjoy the results in {str(out_path)} and {str(mid_out_path)}")
