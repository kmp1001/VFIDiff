...
def pad_to_window(x, window_size):
    """
    将 x pad 到 window_size 的整数倍，并返回原始 H、W。
    x: Tensor[..., H, W]
    """
    B, C, H, W = x.shape
    new_H = ((H - 1) // window_size + 1) * window_size
    new_W = ((W - 1) // window_size + 1) * window_size
    pad_h = new_H - H
    pad_w = new_W - W
    # pad 参数是 (left, right, top, bottom)
    x = F.pad(x, (0, pad_w, 0, pad_h))
    return x, H, W

class PadSwinWrapper(nn.Module):
    def __init__(self, layer: BasicLayer, window_size: int):
        super().__init__()
        self.layer = layer
        self.ws = window_size

    def forward(self, x, emb=None):
        # 1) pad 到 window 大小的整数倍
        B, C, H, W = x.shape
        new_H = ((H - 1) // self.ws + 1) * self.ws
        new_W = ((W - 1) // self.ws + 1) * self.ws
        pad_h = new_H - H
        pad_w = new_W - W
        x_pad = F.pad(x, (0, pad_w, 0, pad_h))

        # 2) 调用原 BasicLayer，只用 x_pad
        out = self.layer(x_pad)
...
class UNetModelSwin(nn.Module):
        ...
        for level, mult in enumerate(channel_mult):
            for jj in range(num_res_blocks[level]):
                ...
                if ds in attention_resolutions and jj==0:
                    ...
                    # 确保img_size是一个有效的二元组
                    img_size = (ds, ds) if isinstance(ds, int) else ds
                    raw = BasicLayer(
                                in_chans=ch,
                    ...
                    layers.append(PadSwinWrapper(raw, window_size))
                self.input_blocks.append(TimestepEmbedSequential(*layers))
                input_block_chans.append(ch)
            if level != len(channel_mult) - 1:
                out_ch = ch
    ...
    def forward(self, x, timesteps, lq=None, mask=None,flow0=None, flow1=None,flow0_original=None,flow1_original=None,first_stage_model=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels)).type(self.dtype)
        # print(f"lq.shape0:{lq.shape}")  #256
        # print(x.shape)
        if lq is not None:
            assert self.cond_lq
            if mask is not None:
            ...
            #print(f"lq.shape0:{lq.shape}")  #256*256
