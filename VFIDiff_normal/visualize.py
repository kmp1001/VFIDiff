...
def compute_optical_flow(model, image1, image2):
    ...
    Args:
        ...
        image1: torch.Tensor, 形状为 [B, 3, H, W]，第一帧图像。
        image2: torch.Tensor, 形状为 [B, 3, H, W]，第二帧图像。
...
def visualize_flow(root_dir, viz_root_dir, model, img_pairs, keep_size):
    ...
    for img_pair in img_pairs:
        ...
        if image1_np.max() <= 1.0:
            image1_np = (image1_np * 255).clip(0, 255).astype(np.uint8)
        else:
            image1_np = image1_np.clip(0, 255).astype(np.uint8)
        ...
        if image2_np.max() <= 1.0:
            ...
            image2_np = image2_np.clip(0, 255).astype(np.uint8)

        diff = np.abs(image2_np.astype(np.int16) - warped.astype(np.int16))
        diff = diff.clip(0, 255).astype(np.uint8)

        diff_fn = viz_fn.replace('.png', '_diff.png')
        print(f"[DEBUG] Saving difference image to: {diff_fn}")
...
def process_sintel(sintel_dir):
    ...
    for scene in os.listdir(sintel_dir):
        ...
        for i in range(len(image_list) - 1):
            fn1 = osp.relpath(image_list[i], start='.')
            fn2 = osp.relpath(image_list[i + 1], start='.')
            img_pairs.append((fn1, fn2))
    return img_pairs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sintel_dir', default='softmax_splatting_master/test_images')
